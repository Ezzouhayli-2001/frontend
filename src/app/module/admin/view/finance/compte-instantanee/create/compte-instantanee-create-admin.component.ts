...
@Component({
  selector: 'app-compte-instantanee-create-admin',
  standalone: false,
  templateUrl: './compte-instantanee-create-admin.component.html'
})
export class CompteInstantaneeCreateAdminComponent  implements OnInit {
	...
	constructor(
        private localeService: LocalAdminService,
        private locationService: LocationAdminService,
        private service: CompteInstantaneeAdminService , private transactionService: TransactionAdminService, private typePaiementService: TypePaiementAdminService, private compteService: CompteAdminService, private modePaiementService: ModePaiementAdminService, private locataireService: LocataireAdminService, private typeTransactiontService: TypeTransactiontAdminService, @Inject(PLATFORM_ID) private platformId? ) {
    ...
    ngOnInit(): void {
        this.localeService.findAll().subscribe((data) => this.locals = data);
    ...
    public save(): void {
        this.submitted = true;
        this.validateForm();
        this.item.location = this.locationService.items.find(e => e.local.code === this.item.locale.code&&e.locataire.code === this.item.locataire.code);
        this.item.code = `CompteInstantanee${Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)}`;
    ...
    cancel() {
        this.router.navigate(['/app/admin/finance/compte-instantanee/list']);
    }
}
